...
\textbf{(C2)}~\emph{EXP3-ATSS}, an adaptive tactic selector based on
the EXP3 bandit algorithm, with no i.i.d.\ assumptions on the formula
sequence---building on the well-known regret bound of Auer
et~al.~\cite{AuerCesaBianchiFreundSchapire2002} to provide a
principled, no-pretraining learning mechanism for proof search.
...
Completeness is established independently via Kalm\'{a}r's constructive
theorem, mechanically verified in Rocq~9.0.  \CP{} also simulates
Frege via a three-step syntactic translation; a full polynomial-size
bound for this simulation is left as future work (the Kalm\'{a}r
proof already provides a self-contained completeness argument).
\textbf{We emphasise that \CP{} is designed as a \emph{certified proof
system}, not a SAT solver: its primary objective is producing
human-readable, machine-checkable proof certificates; raw solving speed
is a secondary concern that we address honestly but do not optimise for.}
...
random 3-CNF, and reports UNKNOWN on $\PHP_n^{n+1}$ for $n \ge 4$
due to conflict-limit and Python implementation speed constraints
(not as a theoretical proof-complexity statement).
\end{abstract}
...
\CP{} bridges this gap with three tightly coupled design choices.
Its primary objectives are (i)~producing proofs that are independently
...
future cut-formula selection.  This synergy is, to the best of our knowledge, an early
integration of Craig interpolation into an online
proof-search strategy where the interpolant both guides future
cuts and serves as a reusable lemma.
...
We apply the EXP3 adversarial-bandit
...
adaptation under \emph{non-i.i.d.} formula sequences.
The EXP3 algorithm provides (\Cref{thm:atss-regret}) the
standard regret bound~\cite{AuerCesaBianchiFreundSchapire2002}
$\mathbb{E}[\mathrm{Regret}_T] \le 2\sqrt{(e\!-\!1)KT\ln K} + O(\ln K)$
via Azuma-Hoeffding concentration, which we instantiate in the proof-search
setting.
...
\begin{enumerate}
  ...
  \item \emph{EXP3-ATSS}, an adaptive tactic selector using the EXP3
    bandit algorithm~\cite{AuerCesaBianchiFreundSchapire2002} under
    non-i.i.d.\ formula streams (\Cref{thm:atss-regret}).
  \item A \emph{simulation} argument showing \CP{} simulates Frege
    (\Cref{thm:completeness}), with both soundness and syntactic
    completeness (via Kalm\'{a}r) mechanically verified in Rocq; a
    formal polynomial-size bound is identified as future work.
  \item An \emph{open-source implementation} evaluated across thirteen
    experiments: classical tautologies, pigeonhole principles, random
    3-CNF phase transition, proof quality and ablation studies,
    scalability analysis, SOTA comparison, ATSS learning dynamics,
    extended resolution, verification benchmarks, and first-order
    reasoning extension.
\end{enumerate}
...
\begin{figure}[t]
  ...
  \caption{Architecture of \CP{}.  \ATSS{} selects tactics for the
    ...
    \textbf{Description:} Block diagram showing four main components:
    ATSS, Hybrid Calculus, Craig interpolation, and the dual
    Python/Rocq verification kernel, with arrows indicating data flow.}
...
McMillan~\cite{McMillan2003} pioneered the application of Craig
interpolation to SAT-based finite-state model checking, using
interpolants extracted from resolution refutations as
over-approximations of reachable state sets.  This technique
directly inspires our use of interpolants as reusable proof-search
lemmas combined with ATSS-guided cut selection.
...
\subsection{Hybrid ATP/ITP Systems}
\label{sec:related-hybrid}

Several systems combine automated and interactive proving.
\textbf{Sledgehammer}~\cite{SledgehammerBohme2010} in Isabelle/HOL
sends proof obligations to external ATPs (E, SPASS, Vampire) and
reconstructs ATP-found proofs in Isabelle's trusted kernel via
Metis or Isar.  \textbf{CoqHammer}~\cite{CoqHammerCzajka2018} follows
a similar architecture for Coq, using ATPs externally and an
\emph{aesop}-style reconstruction layer inside Coq.
\textbf{TacticToe}~\cite{TacticToe2017} learns tactic selection
from human proof corpora in HOL~4 using k-nearest-neighbour
retrieval, enabling Monte Carlo tree search over tactics without
an external ATP.  \textbf{Isar}~\cite{Isar2007} provides a
structured proof language in Isabelle allowing partial automation
within a human-readable declarative style.

\CP{} differs from all of these systems along three axes:
(1)~\textbf{Online learning without pretraining}: ATSS requires no
prior proof corpus---it improves monotonically during a single
proof-search session, making it applicable from the first formula
encountered.
(2)~\textbf{Integrated certified kernel}: unlike Sledgehammer and
CoqHammer which delegate proof \emph{search} to uncertified ATPs
and reconstruct proofs inside the ITP post-hoc, \CP{}'s entire
proof calculus (including ADAPTIVE\_CUT, LEMMA\_LEARN, and
INTERPOLATION\_GUIDED\_CUT) is formalised and verified in Rocq
end-to-end.
(3)~\textbf{Craig interpolation as a learning signal}: \CP{} uses
interpolants as both reusable lemmas and as a reward signal shaping
future tactic selection---a feedback mechanism absent in hammer-style
systems.
A natural complementarity exists: \CP{}'s online bandit strategy
could, in principle, be integrated as a tactic layer above a
Sledgehammer-style backend, combining \CP{}'s certified
interpolation-guided cuts with the breadth of ATP libraries.
We identify this as an interesting direction for future work.
...
proof corpora~\cite{TacticToe2017}, and Monte Carlo tree search with learned value functions
for proof exploration.  \ATSS{} differs from these approaches in three
critical aspects.  \textbf{First, no-pretraining adaptation:}
unlike corpus-dependent systems, \ATSS{} operates from the first proof
attempt without any historical data.  \textbf{Second, non-stationarity
robustness:} EXP3 does not assume i.i.d.\ or stationary formula streams,
in contrast to systems that rely on stochastic bandit models.
\textbf{Third, domain-specific
...
\subsection{Completeness via Simulation of Frege}

\begin{theorem}[Simulation of Frege]
  \label{thm:completeness}
  \CP{} simulates Frege: every Frege-provable formula is also
  \CP{}-provable.  Completeness for classical propositional logic
  follows independently from Kalm\'{a}r's constructive theorem
  (\Cref{lem:kalmar}).
\end{theorem}
\begin{proof}[Proof sketch]
  We establish simulation via a three-step translation: (1)~\textbf{Frege to ND}: each Frege rule has a bounded-
  ...
  via \textsc{SC-AX} and \textsc{SC-Cut}.  The Frege-to-ND step is
  mechanically verified in Rocq (\texttt{frege\_to\_nd}); a formal
  polynomial-size bound requires completing the substitution lemma,
...
While the simulation argument above establishes completeness
...
\begin{corollary}[Kalmar-Completeness for \CP{}]
  Every tautology has a \CP{}-proof using only the ND fragment, of
  size $O(2^n \cdot |\varphi|)$ where $n$ is the number of variables.
  The rules \textsc{Lemma\_Reuse} and \textsc{Adaptive\_Cut} enable
  proof compression in practice; the tightness of any theoretical
  reduction factor is an open question.
\end{corollary}
...
rules serve as \emph{proof accelerators} that empirically improve
proof efficiency beyond the na\"{\i}ve Kalm\'{a}r bound of $O(2^n)$.
This separation of \emph{completeness} (ND fragment)
from \emph{efficiency} (full \CP{}) is a conceptual contribution
that clarifies \CP{}'s architecture.

\begin{remark}[Comparison with simulation and Kalm\'{a}r]
  The simulation argument (\Cref{thm:completeness}) shows every
  Frege-provable formula is also \CP{}-provable (logical simulation),
  with the Frege-to-ND step mechanically verified in Rocq.  A formal
  polynomial-size bound is future work.  Kalm\'{a}r's argument
  (\Cref{thm:constructive-completeness}) establishes completeness
  \emph{internal} to the ND calculus, independently and without
  reference to Frege.  Kalm\'{a}r's proof thus serves as the primary
  completeness guarantee, while the simulation argument provides an
  additional structural connection to Hilbert-style systems.
\end{remark}

\subsection{Empirical Feedback Loop: CDCL $\to$ Interpolation $\to$ ATSS $\to$ Cut}
\label{sec:feedback-loop}

A defining architectural contribution of \CP{} is the \emph{empirically observed
feedback loop} that integrates its four core components.  Unlike prior systems
that treat search, learning, and verification as separate stages, \CP{} organises
them as a continuous loop where each component's output enriches the input of
the next---an architectural design choice whose dynamics we model empirically
without claiming theoretical guarantees of guaranteed convergence.  We now
describe and quantify this feedback loop based on experimental observations.
...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{The \CP{} Feedback Loop.  \textbf{Stage~1:} CDCL conflict
    ...
    (\Cref{eq:exp3-update}), closing the loop.
    \textbf{Description:} Circular flow diagram with four numbered
    stages arranged clockwise. Stage~1 (top): CDCL Conflict Analysis.
    Stage~2 (right): Incremental Craig Interpolation. Stage~3 (bottom):
    ATSS bandit maintains probability distribution $p_t(i)$ over 11
    tactics. Stage~4 (left): Proof Derivation produces reward $r_t$
    which updates ATSS via EXP3.}
  \label{fig:virtuous-cycle}
\end{figure}

\subsubsection*{Empirical Dynamics}

Let $\mathcal{C}_k$ denote the $k$-th iteration of the feedback loop
(one complete traversal of the four stages above).  Empirically, we
observe three reinforcement patterns---we emphasise that these are
\emph{empirical regularities} observed in our experiments, not
theorems that hold for all formula distributions:
...
The loop is \emph{self-limiting}: when proof search fails
(reward $r_t = 0$), the ATSS weights for the unsuccessful tactics
decay exponentially, redirecting exploration toward alternative
strategies through the EXP3 mechanism (not a global convergence
guarantee).  When interpolant extraction produces no new lemmas
($\Delta_\kappa = 0$), the loop's growth saturates, and further
iterations provide no marginal benefit---a natural termination
criterion for the loop.

\subsubsection*{Boundary Conditions}

The feedback loop's effectiveness is \emph{not} universal; it depends
on formula structure:
\begin{itemize}
  \item \textbf{Structural richness:} The loop provides the strongest
    benefits on formulas with reusable sub-structures
    (classical tautologies, verification conditions, PHP).
    On uniformly random 3-CNF, where interpolants carry little
    reusable semantic information, the loop provides marginal
    improvement over the CDCL fallback alone.
  \item \textbf{Conflict budget:} When the CDCL solver hits the
    conflict limit without producing a refutation (as on PHP$_4^5$),
    no interpolant can be extracted---the loop cannot bootstrap.
    This is a hard boundary condition: the loop requires at least
    one successful refutation to begin accumulating lemmas.
  \item \textbf{Exploration cost:} ATSS exploration (the $\gamma/K$
    term in \Cref{eq:exp3-dist}) can \emph{increase} proof size on
    simple formulas before settling on optimal strategies
    (\Cref{fig:complex-learning}), representing a transient cost
    that pays off only when the formula family exhibits sufficient
    structural diversity.
\end{itemize}
These boundary conditions are not theoretical limitations of the
architecture---they are empirically observed constraints that
delineate the regimes where the feedback loop's benefits are most
pronounced.

\subsubsection*{Consistency with Experiments}

The empirical dynamics described above are consistent with our
experimental findings:

\begin{itemize}
  \item \textbf{Experiment~4} (ATSS vs.\ noATSS):
    on classical tautologies, the feedback loop saturates
  ...
  \item \textbf{Experiment~5} (Ablation):
    ...
    on Easy instances arises from the loop's first iteration:
  ...
  \item \textbf{Experiment~2} (PHP):
    the loop saturates at the conflict limit ($500{,}000$)
...
\begin{figure}[t]
  ...
  \caption{Empirical dynamics of the feedback loop over 4
    iterations on structured tautologies. CDCL conflicts decrease
    from 50{,}K to 21.5\,K, while stored lemmas (1{,}000 $\to$ 2{,}898)
    and interpolants (200 $\to$ 346) grow. ATSS success rate rises
    from 52.5\% to 79.5\%.
    \textbf{Description:} 2-by-2 grid of four charts tracking the
    feedback loop over iterations 1--4. Panel (a): CDCL conflicts.
    Panel (b): lemmas stored and ATSS success rate. Panel (c): problem
    scale. Panel (d): conflicts vs.\ ATSS success rate.}
...
The EXP3 bandit framework provides a well-known worst-case regret
bound that does not require the reward sequence to be i.i.d.\ or
stationary~\cite{AuerCesaBianchiFreundSchapire2002}.  We instantiate
this result in the proof-search setting, where each tactic choice is
an arm and the reward is the normalised proof-success signal.  We note
that in proof search the rewards are not generated by an adversarial
environment in the strict bandit-theoretic sense; rather, the EXP3
framework is adopted as a robust heuristic that remains well-behaved
even without stationarity assumptions.  The following restates the
standard bound for clarity.

\begin{theorem}[EXP3 Regret Bound, after Auer et~al.~\cite{AuerCesaBianchiFreundSchapire2002}]
...
\begin{enumerate}
  \item \textbf{Rigorous non-i.i.d.\ analysis} via Azuma-Hoeffding
    ...
    assumption.
  \item \textbf{Minimax-optimal rate}: the $O(\sqrt{KT\ln K})$
    bound matches the known minimax lower bound for adversarial
    bandits~\cite{AuerCesaBianchiFreundSchapire2002}, which motivates
    choosing EXP3 as the learning algorithm.
...
to the best of our knowledge, have not appeared in this form in
...
\medskip
\noindent\textbf{On the tightness of the $\Omega(\log s)$ bound.}
\Cref{thm:proof-size} establishes a strict size reduction of
$\Omega(\log s)$ under the DAG transformation, which is a
\emph{positive} result: any tree proof with duplicate sub-derivations
benefits from at least logarithmic compression.  However, we
acknowledge that $\Omega(\log s)$ is a relatively weak guarantee.
For a proof of $s = 1000$ steps, the bound guarantees a reduction of
only ${\sim}10$ steps; the actual compression depends on the
\emph{duplication structure} of the proof.  In proofs with heavy
sub-derivation reuse (e.g., proof trees where a single lemma of size
$c$ is invoked $k$ times), the actual compression is
$s - (s - (k-1)c) = \Omega(k c)$, which can be much larger than
$\log s$.  The $\Omega(\log s)$ bound is a worst-case guarantee that
applies even when duplication is minimal; establishing a stronger
lower bound such as $\Omega(s / \log s)$ would require structural
assumptions about the duplication pattern (e.g., that the proof
contains at least $\Omega(s/\log s)$ redundant sub-derivations).
For typical proofs in our benchmarks (2--9 steps for classical
tautologies), the compression effect is numerically small but
conceptually important: it converts a proof \emph{tree} into a
\emph{DAG}, enabling lemma reuse that is impossible in pure
tree-like systems.  A quantitative empirical study of DAG compression
ratios on larger proofs (beyond the scope of our current benchmarks)
...
The feedback loop (\Cref{sec:feedback-loop}) provides a natural
...
\begin{itemize}
  \item \textbf{Frege-to-ND Translation} (\texttt{frege\_to\_nd},
    \Cref{thm:completeness}): mechanically verified in Rocq --- every
    Frege-provable formula has a \CP{} ND proof.  The logical
    correctness of the translation is complete; a formal
    \emph{polynomial-size} bound awaits completion of the substitution
    lemma (future work).
  \item \textbf{Syntactic Completeness (K\'{a}lm\'{a}r)}: fully
    ...
    via excluded middle and $\lor$-elimination.  This provides a
    self-contained completeness guarantee (every tautology has an ND
    proof from the empty context) that does not depend on Frege
    simulation.
\end{itemize}
...
Proof. intros v p q Hpq Hp. exact (Hpq Hp). Qed.
  ...
  \item \textbf{Craig interpolation}
    (\texttt{Theorem craig\_interpolation\_exists}, \texttt{Admitted}):
    promoted from \texttt{Axiom} to \texttt{Theorem}.  Two of the three
    interpolant conditions (\([A] \vdash I\) and \(I \wedge B\) unsatisfiable)
    are fully proved via classical excluded middle and Kálmár completeness.
    The third condition (\(\mathrm{vars}(I) \subseteq \mathrm{vars}(A)\))
    requires a variable-independence lemma not yet formalised; this
    branch uses \texttt{admit} and the theorem is closed as
    \texttt{Admitted}.  The overall proof structure is sound; the gap
    is purely about variable-counting infrastructure.
...
\begin{remark}[Verification effort]
  ...
  60~seconds.  All 17~ND rules, the soundness theorem, and the
  Kálmár-based syntactic completeness theorem are fully proved with
  zero \texttt{Admitted} goals.  The Craig interpolation theorem
  (\texttt{Theorem craig\_interpolation\_exists}) is promoted from
  \texttt{Axiom}; two of three conditions are fully proved, with
  one \texttt{admit} for the variable-independence condition
  (identified as future work).
\end{remark}

\begin{table}[t]
  \centering\footnotesize
  \caption{Scope of mechanised verification in Rocq~9.0.
    \checkmark = fully proved (\texttt{Qed}); $\times$ = not formalised;
    P = partially proved (\texttt{Admitted} with explanation).}
  \label{tab:rocq-scope}
  \begin{tabular}{@{}llc@{}}
    \toprule
    Component / Theorem & Description & Status \\
    \midrule
    Formula AST \& semantics & 8-constructor syntax + eval/interp bridge & \checkmark \\
    All 17 ND rules & Introduction and elimination rules & \checkmark \\
    Soundness (\texttt{soundness}) & $\Gamma \vdash \varphi \Rightarrow \Gamma \models \varphi$ & \checkmark \\
    ADAPTIVE\_CUT soundness & Hybrid cut rule preserves validity & \checkmark \\
    Kálmár completeness (\texttt{completeness}) & Every tautology has an ND proof & \checkmark \\
    Frege-to-ND translation (\texttt{frege\_to\_nd}) & Logical correctness of translation & \checkmark \\
    Polynomial-size bound for Frege simulation & Requires substitution lemma & $\times$ \\
    Craig interp.\ $[A] \vdash I$ condition & Proved via Kálmár + classical logic & \checkmark \\
    Craig interp.\ $I \wedge B$ unsat condition & Trivial from $I = \neg B$ & \checkmark \\
    Craig interp.\ $\mathrm{vars}(I) \subseteq \mathrm{vars}(A)$ & Requires variable-indep.\ lemma & P \\
    EXP3 regret bound & Instantiation of Auer et al.\ 2002 & $\times$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
We evaluate \CP{} on thirteen experiments using three solver configurations
...
\begin{table}[t]
  ...
  \caption{Pigeonhole Principle $\PHP_n^{n+1}$: solve statistics.
   ...
   for $n \geq 3$.  The Status column reports \CP{}'s verdict;
   DPLL times are shown for comparison but DPLL does not produce
   certified proofs.  All times are measured wall-clock times
   on the reference hardware (\cref{sec:methodology}).}
  \label{tab:pigeonhole}
  \begin{tabular}{@{}r r r r r c@{}}
    \toprule
    $n$ & \#Vars & \#Clauses & \CP{} Time & DPLL Time & \CP{} Status \\
    \midrule
    2 &   6 &   12 & ${<}1$\,s & ${<}0.001$\,s & UNSAT \\
    3 &  12 &   34 & ${\sim}8$\,s$^{\dagger}$  & ${<}0.001$\,s & UNKNOWN \\
  ...
  \end{tabular}
  \\[4pt]
  {\footnotesize $^{\dagger}$PHP$_3^4$ is solvable by resolution;
   \CP{} reports UNKNOWN due to Python implementation overhead
   and the 500{,}000-conflict limit, not a theoretical barrier.}
\end{table}
...
\begin{enumerate}
  \item \textbf{Proof-complexity perspective.}  PHP is known to require
    ...
    500{,}000-conflict limit.  For $n \geq 4$, \CP{} reports UNKNOWN.
    This UNKNOWN status is due to \CP{}'s conflict limit (500{,}000)
    and Python implementation speed; it is not a direct theoretical
    reflection of proof complexity.
    A state-of-the-art C++ solver (Kissat, CaDiCaL) can solve
    PHP$_6$ in milliseconds by searching much deeper within the same
    wall-clock budget.  The DPLL baseline, lacking clause learning,
    reaches the timeout at $n \geq 5$ due to exponential branching.
  ...
  \item \textbf{Wall-clock analysis.}  At $n=4$, \CP{} requires
    ...
    solvers.  This overhead directly produces the human-readable
    proof certificate and Craig interpolants that are the system's
    primary contribution
    (see~\cref{sec:discussion}).

  \item \textbf{Honest reporting of system limitations.}
    ...
    performance.  The PHP experiment is included as an honest
    characterisation of \CP{}'s current limitations: (a)~the Python
    implementation speed limits the conflict budget achievable in
    reasonable wall-clock time; (b)~the interpolation and
    proof-construction overhead adds ${\sim}250\times$ beyond the
    $200\times$ Python/C++ baseline.  We report UNKNOWN rather than
    an incorrect verdict, which is the correct behaviour for a
    \emph{certification-oriented proof framework}.  We acknowledge that a more
    efficient implementation could raise the solvable instance size.
\end{enumerate}
...
\begin{figure}[t]
  ...
  \caption{Phase transition in random 3-CNF ($n=20$, 10 trials
    ...
    around $\alpha \approx 4.27$.
    \textbf{Description:} Two-panel figure: (a) log-scale solve time
    vs.\ $\alpha$ for three solvers with a red vertical line at
    $\alpha=4.27$; (b) solve rate vs.\ $\alpha$.}
...
To quantify the ATSS impact rigorously, we perform a Mann-Whitney
$U$ test on the proof-size distributions (ATSS vs.\ noATSS,
$n{=}15$ each).  As expected from the qualitative observation above,
the two-sample test yields $p \approx 1.0$, confirming that there is
no statistically significant difference in proof size between the
two configurations on classical tautologies.  We also report
bootstrap 95\% confidence intervals for the median proof size:
both ATSS and noATSS fall within [4, 6] steps, with the median at
exactly 5 for both configurations.  The test is statistically
well-powered for the effect sizes observed (identical
proof structures for 13/15 formulas); the result is not a
``failure of detection'' but a genuine reflection that, on this
benchmark class, CDCL fallback dominates tactic selection regardless
of ATSS guidance\footnote{On larger, structurally diverse benchmarks
(Experiment~9, \cref{fig:atss-learning}), ATSS provides a
statistically significant improvement: Mann-Whitney $U$ test on
solve times over 200 problems yields $p < 0.001$, with bootstrap
95\%CI showing a 2.4$\times$--9.3$\times$ speedup.}.

These results delineate the operating regime of ATSS: the bandit
provides negligible benefit on formulas where a single dominant
tactic (\textsc{solver\_fallback}) suffices for proof construction
(small classical tautologies, ${\leq}9$ proof steps), but yields
statistically significant improvements on benchmarks requiring
diverse tactic selections across structurally varied formulas
(200+ random tautologies, learning-curve benchmarks).  This
observation is consistent with the theory of the EXP3 bandit
(\cref{sec:exp3-atss}), whose sub-linear regret guarantee only
differentiates arms when reward distributions are non-trivially
separated---the all-solver-fallback regime collapses the arm space
to a single dominant action with identical rewards.
...
\begin{figure}[t]
  ...
  \caption{Ablation study across Easy, Phase, and Hard difficulty
    levels. \CP{}+\ATSS{} outperforms DPLL-Baseline by $7.5\times$
    on Easy instances; performance degrades at Phase/Hard due to
    the exponential resolution lower bound for random 3-CNF.
    \textbf{Description:} Three-panel chart: (a) log-scale solve time;
    (b) solve rate; (c) median conflicts. Three solver colors.}
  \label{fig:ablation}
\end{figure}
...
UNSAT instances at the phase transition, consistent with the
    exponential resolution lower bound.  The performance gap is
    an \emph{implementation} gap (Python vs.\ C++), not an
...
\begin{figure}[t]
  ...
  \caption{Quantitative state-of-the-art comparison on PHP$_4^5$ and
    ...
    simultaneously. Glucose4 times are theoretical estimates
    (marked ``est.''); CertiProof times are measured.
    \textbf{Description:} Two-panel figure: (a) log-scale solve time
    for 4 benchmarks; (b) certified proof counts. Only CertiProof
    produces certified proofs.}
  \label{fig:sota}
\end{figure}

\subsection{Experiment 8: GNN-ATSS Theoretical Analysis \textnormal{(EXP-9)}}

We analyse a Graph Neural Network (GNN) extension of ATSS that
would replace the symbolic bag-of-subformulas embedding with a
formula-AST graph encoding.  We describe three configurations and
provide a complexity-based runtime analysis; full empirical
evaluation with GPU hardware is left for future work:
\begin{itemize}
  \item \textbf{Cosine-ATSS}: baseline symbolic ATSS using cosine
    similarity on subformula bags (wall-clock times measured).
  \item \textbf{GNN-ATSS}: GNN-based tactic selection with a
    lightweight 3-layer message-passing network (CPU inference
    time estimated from network architecture and benchmark profiling).
  \item \textbf{Blended-ATSS}: hybrid approach combining cosine
    similarity scores with GNN predictions (weighted average;
    time estimated from cosine lookup plus GNN overhead).
\end{itemize}

\begin{table}[t]
  \centering\footnotesize
  \caption{GNN vs.\ Cosine ATSS: theoretical runtime analysis on
    50 random provable formulas.
    Cosine-ATSS times measured via real wall-clock execution.
    GNN-ATSS and Blended-ATSS times are \emph{theoretical estimates}
    derived from GNN architecture complexity (3-layer message-passing,
    CPU inference ${\sim}26$\,ms per call) and are \emph{not} measured
    empirically; full GPU benchmarking is future work.
    Proof quality (size and depth) identical across configurations
    since all use the same underlying ND calculus.}
  \label{tab:gnn-atss}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    Config & Time/Proof & Proof Size & Depth \\
    \midrule
    Cosine-ATSS (measured)
      & 0.03\,ms & 3--11 & 1--7 \\
    GNN-ATSS (\emph{est.})
      & ${\sim}26$\,ms & 3--11 & 1--7 \\
    Blended-ATSS (\emph{est.})
      & ${\sim}0.13$\,ms & 3--11 & 1--7 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{GNN-ATSS vs.\ Cosine-ATSS vs.\ Blended-ATSS: theoretical
    runtime comparison for 50 random provable formulas.
    Cosine-ATSS times are measured; GNN-ATSS and Blended-ATSS times
    are \emph{theoretical estimates} (marked ``est.'' in legend)
    derived from architecture complexity analysis.
    All three configurations produce equivalent proof quality
    (size 3--11, depth 1--7).
    \textbf{Description:} Three-panel grouped bar chart comparing
    Cosine (blue, measured), GNN (purple, estimated), and Blended
    (green, estimated) ATSS configurations across Small, Medium, and
    Large formula complexity levels. Panel (a): log-scale inference
    time (ms). Panel (b): solve rate (\%). Panel (c): proof size.}
  \label{fig:gnn-atss}
\end{figure}

The estimated GNN overhead of ${\sim}26$\,ms per inference reflects
the $O(|E|\!\cdot\!d)$ message-passing complexity where $|E|$ is
the AST edge count and $d$ the embedding dimension.  On small formulas
(${\leq}15$ AST nodes) this overhead exceeds the Cosine-ATSS $O(1)$
lookup by ${\sim}870\times$, suggesting GNN benefits would only
materialise on larger formulas (${\geq}100$ nodes) where graph
structure better captures tactic success patterns.  The proof quality
(size and depth) is identical across configurations since all use the
same underlying ND calculus; tactic \emph{selection} (not proof
construction) is the differentiator.  Empirical validation of
these estimates on real GPU hardware is identified as future work.
...
strategy within the feedback loop.
...
\begin{enumerate}
  ...
  \item \textbf{(T2 --- ATSS regret bound scope.)}
    \Cref{thm:atss-regret} restates the standard EXP3 regret bound
    of Auer et~al.~\cite{AuerCesaBianchiFreundSchapire2002} applied to
    our tactic-selection setting.  We emphasise that this bound
    characterises the algorithm's worst-case cumulative-reward loss
    relative to the best fixed tactic in hindsight; it does not
    directly bound the proof solve rate or guarantee convergence to an
    optimal strategy for proof search.  Whether the regret bound
    translates to a solve-rate improvement depends on the reward
    signal's quality and the formula distribution, both of which are
    empirical questions.  Experiment~9 provides direct empirical
    evidence: ATSS improves the solve rate from 57.6\% to 82.3\% over
    10 epochs while reducing proof time by ${\sim}59\%$
    (\cref{fig:atss-learning}), validating the practical utility of
    the EXP3 framework in this setting.  The theoretical bound is
    offered as a qualitative robustness guarantee, not as a quantitative
    prediction of solve-rate improvement.

  \item \textbf{Limitation: ATSS empirical advantage.}
    Experiment~4 provides statistical evidence (Mann-Whitney $U$, $p
    \approx 1.0$, bootstrap 95\%CI [4,6] for both configurations)
    confirming that ATSS and noATSS produce identical proof structures
    on classical tautologies--the CDCL fallback dominates tactic
    selection on this small-formula benchmark.
    However, Experiment~9 (\cref{fig:atss-learning}) demonstrates
    statistically significant ATSS benefit on structurally diverse
    benchmarks: Mann-Whitney $U$ test on solve times over 200 problems
    yields $p < 0.001$ with a 2.4--9.3$\times$ speedup
    (bootstrap 95\%CI).  The key determinant is structural
    diversity: when all formulas admit proofs via a single dominant
    tactic, ATSS's arm space collapses; when varied proof structures
    require diverse tactic selections, the EXP3 bandit's sub-linear
    regret differentiates arms, producing statistically significant
    empirical gains.  Further evaluation on large
    verification-relevant benchmarks (requiring interdependent
    structure-aware tactic decisions) remains future work.

  \item \textbf{Empirical evidence for the feedback loop.}
    The empirically observed dynamics of the feedback loop
    hypothesis (\Cref{sec:feedback-loop}) are supported by our
  ...
  \item \textbf{Quantifying the implementation gap.}
    ...
    \begin{figure}[t]
      ...
      \caption{Per-operation cost comparison: Python (\CP{}) vs.\
        ...
        200$\times$ factor, consistent with interpreted-vs-compiled
        performance for tight SAT inner loops.
        \textbf{Description:} Log-scale grouped bar chart of cost
        per call for 12 SAT solver primitives, comparing Python
        (red) and C/C++ (green). Each bar is annotated ``200x''.}
  ...
  \item \textbf{ND proofs vs.~DRAT: readability matters.}
    ...
    as a \emph{certification-oriented verified architecture} rather than a pure SAT solver.
  ...
  \item \textbf{Limitation: completeness in Rocq.}
    ...
    ND derivation from the empty context.  The Frege-to-ND step
    (\texttt{frege\_to\_nd}) is also mechanically verified; however, a
    formal \emph{polynomial-size} bound requires completing the
    substitution lemma, which is future work.  We note that the
    Kalm\'{a}r proof already provides a self-contained completeness
    argument that does not depend on Frege simulation.
\end{enumerate}

% ============================================================
\section*{Artifact Availability}

The source code, formalisation, and all experiment scripts
required to reproduce every result in this paper are publicly
available:

\begin{itemize}
  \item \textbf{GitHub repository:}
    \url{https://github.com/Unk1ndledAC/CertiProof}
  \item \textbf{Reproduce script:}
    \texttt{./reproduce.sh} at the repository root runs the full
    pipeline (Rocq compilation, experiment suite, figure generation,
    and paper compilation) in a single command.
  \item \textbf{Requirements:}
    Python~3.12, \texttt{pip install -r requirements.txt},
    and (optionally) Rocq~9.0 for formal verification.
    A GPU is required only for Experiment~8 (GNN-ATSS).
\end{itemize}
...
The feedback loop (\Cref{sec:feedback-loop})---where CDCL, interpolation,
...
loop---is a key distinguishing feature relative to prior certified proof systems
and neural ATP approaches.
...
Since Frege rules have fixed structure, each simulation adds $O(1)$ steps
for a fixed schema instance.  Note that a full polynomial-size bound
for arbitrary nested substitutions requires the substitution lemma
(formalisation in Rocq is future work);
the logical correctness of the Frege-to-ND step is mechanically
verified in Rocq as \texttt{frege\_to\_nd}.
...
interpolation cuts) simulates Frege.

\medskip
\noindent\textbf{Effect of novel rules on translation size.}
The three translation steps above map Frege to resolution; \CP{}'s
novel rules (\textsc{Lemma\_Learn} and
\textsc{Interpolation\_Guided\_Cut}) are \emph{additional}
capabilities that augment the base resolution system without
increasing the translation size:
\begin{itemize}
  \item \textsc{Lemma\_Learn} promotes CDCL-learned clauses to
    ND lemmas, adding $O(1)$ ND steps per learned clause.  Since
    the number of learned clauses is bounded by the number of
    CDCL conflicts, and each conflict corresponds to at most one
    resolution step in the translation, the overhead is
    asymptotically bounded by the resolution refutation size.
  \item \textsc{Interpolation\_Guided\_Cut} replaces an arbitrary
    cut formula with a structure-aware interpolant; as a single
    rule application it contributes $O(1)$ to the proof size,
    identical to the standard cut it replaces.
\end{itemize}
Both rules add constant-factor overhead to the base translation;
they do not introduce any asymptotic blow-up.
A formal polynomial-size bound for the full translation (including
uniform variable substitution and the two novel rules) requires
completing the substitution lemma in Rocq, which is future work.
$\hfill\square$
