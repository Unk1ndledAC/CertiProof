...
%\author{
%    Guanheng Qu \and Chunxiao Zhang\and Jiangming Liu\thanks{Corresponding author. Email: jiangmingliu@ynu.edu.cn}
%}

\author{
\IEEEauthorblockN{Guanheng Qu\textsuperscript{1}, Chunxiao Zhang\textsuperscript{2}, Jiangming Liu\textsuperscript{1*}}
\IEEEauthorblockA{\textit{\textsuperscript{1}School of Information Science and Engineering, Yunnan University, Kunming, China}\\
\textit{\textsuperscript{2}School of Software Engineering, East China Normal University, Shanghai, China}\\
\textsuperscript{*}Corresponding author: jiangmingliu@ynu.edu.cn}
}
...
\begin{proof}[Proof sketch]
  ...
  length \NP{} simulation---modus ponens is exactly \textsc{MP};
  ...
  with at most linear blow-up, and \NP{} provides \textsc{SC-Cut} for the
  sequent calculus cut rule.  (3)~\textbf{Sequent calculus to \NP{}}
...
A defining architectural contribution of \NP{} is the \emph{virtuous
...
and verification as separate stages, \NP{} organises them as a
...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{The \NP{} Virtuous Cycle.  \textbf{Stage~1:} CDCL conflict
...
\begin{itemize}
  ...
  \item \textbf{Experiment~5} (Ablation):
    the $7.5\times$ speedup of \NP{}+\ATSS{} over DPLL-Baseline
...
We evaluate \NP{} on ten benchmarks using three solver configurations
...
\begin{figure}[t]
  ...
  \caption{Operation-primitive cost comparison: Python vs.\ estimated
    C/C++ across 10 core operations. The ${\sim}200\times$ gap reflects
    Python's dict-based data structures vs.\ C arrays. Interpolant and
    conflict-analysis operations dominate \NP{}'s runtime.}
  \label{fig:op-costs}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Classical tautology proofs: size and depth distribution
    ...
    (\NP{}+\ATSS{}), with proof sizes 2--9 steps and depths 1--5.}
...
\begin{figure}[t]
  ...
  \caption{Pigeonhole principle $\PHP_n^{n+1}$: solve time and
    conflict count scaling with $n$. \NP{}+\ATSS{} correctly
...
\begin{figure}[t]
  ...
  \caption{Proof quality comparison: \NP{}+\ATSS{} vs.\ \NP{}
...
\begin{figure}[t]
  ...
  \caption{Ablation study across three difficulty levels
    ...
    \NP{}+\ATSS{} outperforms DPLL-Baseline by $7.5\times$ on
...
\begin{figure}[t]
  ...
  \caption{Quantitative state-of-the-art comparison on PHP$_4^5$ and
    random 3-CNF ($\alpha{=}4.0$). \NP{} is the only system achieving
    certified proofs, online learning, and zero pre-training
    simultaneously. Operation counts (not wall-clock time) enable
...
\NP{}+\ATSS{} against \NP{} with ATSS disabled across 100 random
...
resolution~\cite{CookReckhow1979,Krajicek1995}.  While \NP{}'s
default CDCL kernel does not yet include the extension rule in
the certified proof checker, this experiment demonstrates the
potential of incorporating extended resolution as a certified
inference step within the hybrid calculus.
...
\begin{enumerate}
  ...
  \item \textbf{Empirical evidence for the virtuous cycle.}
    The virtuous cycle hypothesis (\Cref{sec:virtuous-cycle})
    makes three falsifiable predictions, and our experiments provide
    supporting evidence for each
    (\cref{fig:virtuous-cycle-data}).
    \textbf{(i)~Interpolant growth:}
    the number of distinct interpolants in the lemma table grows
    monotonically with cycle iterations on structured tautologies
    (200 $\to$ 346 over 4 cycles, +73\%); on random 3-CNF at the
    phase boundary, interpolant coverage stabilises within 5--8 cycles,
    consistent with the lack of reusable structure in random instances.
    \textbf{(ii)~ATSS convergence:}
    Experiment~4 shows limited differentiation on classical
    tautologies (where the CDCL fallback subsumes search),
    but Experiment~9 provides direct evidence for ATSS learning on
    structurally varied random instances (solve rate 57.6\% $\to$
    82.3\% over 10 epochs), and Experiment~5 demonstrates
    a $7.5\times$ speedup of \NP{}+\ATSS{} over DPLL-Baseline on
    Easy instances---this gain \emph{depends on} the lemma table
    populated by the interpolation stage of the cycle.
    \textbf{(iii)~Lemma reuse growth:} the fraction of
    proof steps invoking \textsc{Lemma\_Reuse} or
    \textsc{Lemma\_Learn} increases from $\rho_0 = 0$ (first cycle)
    to $\rho_k \approx 0.3$--$0.5$ within 5--10 cycles on structured
    tautologies, as predicted by \Cref{eq:reuse-rate}.  On random
    3-CNF, the reuse rate plateaus at ${\sim}0.1$--$0.2$, reflecting
    the entropy of random clause structure.
    The cycle dynamics (\cref{fig:virtuous-cycle-data})
    quantitatively confirm reduced CDCL conflicts ($-57\%$ over
    4 iterations) and rising ATSS success rates (52.5\% $\to$
    79.5\%), achieving 3 out of 4 score-improvement prediction
    points.  Full validation on verification-scale benchmarks
    remains future work.
  ...
  \item \textbf{Experimental scope.}  Our ten experiments cover
    ...
    online learning dynamics (Exp.~9), and extended
    resolution evaluation (Exp.~10).  This
...
The virtuous cycle is the key architectural insight of \NP{}:
CDCL provides raw material for interpolation, which feeds ATSS
with semantically meaningful lemmas, which guides proof search
toward efficient decompositions via
\textsc{Lemma\_Learn} and \textsc{Interpolation\_Guided\_Cut},
which in turn rewards ATSS with positive proof-closure signals,
closing the loop (see \Cref{sec:virtuous-cycle} for quantitative
analysis).  This integrated feedback architecture---where search
(CDCL), learning (ATSS), lemma extraction (interpolation), and
verification (dual Python/Rocq TCB) are not separate modules
but mutually reinforcing stages of a single closed loop---is
what distinguishes \NP{} from every existing proof system.
...
Hence \NP{} (which extends resolution with lemma learning and
