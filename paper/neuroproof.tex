...
\begin{enumerate}
  ...
  \item \textbf{Empirical evidence for the virtuous cycle.}
    ...
    supporting evidence for each.  \textbf{(i)~Interpolant growth:}
    ...
    on random 3-CNF at the phase boundary, interpolant coverage
    stabilises within 5--8 cycles, consistent with the lack of
    reusable structure in random instances.  \textbf{(ii)~ATSS
    convergence:} although Experiment~4 shows limited differentiation
    between ATSS and noATSS on classical tautologies (where the CDCL
    fallback subsumes search), the ablation study (Experiment~5)
    demonstrates a $7.5\times$ speedup of \NP{}+\ATSS{} over
    DPLL-Baseline on Easy instances---this gain \emph{depends on}
    the lemma table populated by the interpolation stage of the
    cycle.  \textbf{(iii)~Lemma reuse growth:} the fraction of
    ...
    the entropy of random clause structure.  These observations are
    \emph{qualitatively} consistent with the cycle model; a rigorous
    quantitative validation---including high-resolution measurement of
    the per-cycle improvement rate in ATSS success probability---is
    planned for future work on benchmarks where the CDCL fallback does
    not dominate (e.g., circuit verification conditions, bit-vector
    equivalence proofs).
