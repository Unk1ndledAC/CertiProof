...
\begin{table}[t]
  ...
  \caption{Pigeonhole Principle $\PHP_n^{n+1}$: solve statistics.
   ...
   the exponential resolution lower bound.  Times are approximate;
   re-measurement with consistent hardware is planned.}
...
with small constant-size proofs (2--10 steps), the CDCL kernel
...
per-proof time because it relies primarily on the cosine-similarity
path; the GNN is invoked only as a tie-breaker when cosine scores
are nearly equal, a rare occurrence on these small formulas.  This
makes Blended-ATSS behaviourally close to Cosine-ATSS in this
regime, with the small measured speedup attributable to measurement
noise on sub-millisecond timings.  We anticipate structural advantages
of the GNN component on larger formulas where graph embeddings better
capture tactic success patterns than bag-of-subformulas overlap.
